import assumptionController from "./assumptionController.js";
...
describe("assumptionController", () => {
    it("should update returns and assets mix assumptions for a valid user and valid input", async () => {
        ...
        expect(res._getJSONData()).toEqual({ message: "Returns & Assets Mix Assumptions are set" });
    ...
    it("should return 403 if input is invalid", async () => {
        ...
        ramSchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        const req = createRequest({user: "67b82107183c091d4d3990c3", body: mockBody});
        const res = createResponse();
        await assumptionController(req, res);

        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "Inputs are incorrect", err: "Validation Error" });
        expect(ramSchema.safeParse).toHaveBeenCalledWith(mockBody);
    });
        ...
        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "Returns & Assets Mix Assumptions are NOT set" });
    });
