import domesticEquityController from "./domesticEquityController.js";
...
describe("domesticEquityController", () => {
    it("should update domestic equity data with valid input and selection", async () => {
        ...
        domesticEquitySchema.safeParse.mockImplementation(({directStocks, mutualFunds, sipEquity}) => {
        ...
        expect(res._getJSONData()).toEqual({ message: "Domestic Equity updated successfully to Networth" });
    ...
    it("should return 403 if input is invalid", async () => {
       const mockBody = { directStocks: "invalid", selection: "directStocks" };
        ...
        domesticEquitySchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        const req = createRequest({user: "67b82107183c091d4d3990c3", body: mockBody});
        const res = createResponse();
        await domesticEquityController(req, res);

        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "Domestic Equity inputs are wrong", err: "Validation Error" });
        expect(domesticEquitySchema.safeParse).toHaveBeenCalled();
    });
    ...
    it("should return 403 if domestic equity data is NOT updated", async () => {
        const mockUserId = "67b82107183c091d4d3990c3";
        const mockUser = { _id: mockUserId, netWorth: { domesticEquity: "67b82107183c091d4d3990ca" } };
        const mockBody = { directStocks: { stock1: 100 }, selection: "directStocks" };
        const mockSession = {
            startTransaction: jest.fn(),
            commitTransaction: jest.fn(),
            abortTransaction: jest.fn(),
        ...
        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "Domestic Equity not updated" });
    });
