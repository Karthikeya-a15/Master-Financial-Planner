...
import cashFlowsController from "./cashFlowsController.js";
...
describe("cashFlowsController", () => {
    it("should update cash flows for a valid user and valid input", async () => {
        ...
        const mockBody = { inflows: { salary: 6000 }, outflows: { expenses: 2500 } };
        const mockExistingCashFlow = { inflows: { other: 1000 }, outflows: { rent: 1000 } }; // Example existing data
        ...
        expect(res._getJSONData()).toEqual({ message: "Cash-Flows Updated successfully" });
    ...
    it("should return 403 if input is invalid", async () => {
        ...
        cashFlowsSchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        ...
        expect(res._getJSONData()).toEqual({ message: "Cash-Flow inputs are wrong", err: "Validation Error" });
        expect(cashFlowsSchema.safeParse).toHaveBeenCalledWith(mockBody);
    });
