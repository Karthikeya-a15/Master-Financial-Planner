...
import foreignEquityController from "./foreignEquityController.js";
...
describe("foreignEquityController", () => {
    it("should update foreign equity data for a valid user and valid input", async () => {
        ...
        const mockExistingForeignEquity = { sAndp500: 10, otherETF: 5, mutualFunds: 10 }; // Example existing data
        ...
        ForeignEquity.findOneAndUpdate.mockImplementation(() => ({
            ...
            sAndp500: 25, otherETF: 12, mutualFunds: 18  //the *updated* values
        }));
        ...
        expect(res._getJSONData()).toEqual({ message: "ForeignEquity updated successfully to Networth" });
    ...
    it("should return 403 if input is invalid", async () => {
        ...
        foreignEquitySchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        ...
        expect(res._getJSONData()).toEqual({ message: "ForeignEquity inputs are wrong", err: "Validation Error" });
        expect(foreignEquitySchema.safeParse).toHaveBeenCalledWith(mockBody);
    });
    ...
    it("should return 403 if foreign equity data is NOT updated", async () => {
        ...
        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "ForeignEquity not updated" });
    });
