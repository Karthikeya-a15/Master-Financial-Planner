...
describe("goldController", () => {
    it("should update gold data for a valid user and valid input", async () => {
        ...
        GoldModel.findOneAndUpdate.mockImplementation(() => ({
          ...
          jewellery: 6, SGB: 3, digitalGoldAndETF: 4
        }));
        ...
        expect(res._getJSONData()).toEqual({ message: "Gold updated successfully to Networth" });
    ...
    it("should return 403 if input is invalid", async () => {
        ...
        goldSchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        ...
        expect(res._getJSONData()).toEqual({ message: "Gold inputs are wrong", err: "Validation Error" });
        expect(goldSchema.safeParse).toHaveBeenCalledWith(mockBody);
    });
