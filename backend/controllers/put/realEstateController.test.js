import realEstatesController from "./realEstateController.js";
...
describe("realEstatesController", () => {
    it("should update real estate data for a valid user and valid input", async () => {
        ...
        const mockUser = { _id: mockUserId, netWorth: { realEstate: "67b82107183c091d4d3990d4" } };
        const mockBody = { home: 120, otherRealEstate: 60, REITs: 20 };
        const mockExistingRealEstate = { home: 100, otherRealEstate: 50, REITs: 15 };
        ...
        RealEstate.findOneAndUpdate.mockImplementation(() => ({
          ...
          home: 120, otherRealEstate: 60, REITs: 20
        }));
        ...
        expect(res._getJSONData()).toEqual({ message: "Real-Estate updated successfully to Networth" });
    ...
    it("should return 403 if input is invalid", async () => {
        ...
        realEstateSchema.safeParse.mockReturnValue({ success: false, error: mockValidationError });
        const req = createRequest({user: "67b82107183c091d4d3990c3", body: mockBody});
        const res = createResponse();
        await realEstatesController(req, res);

        expect(res.statusCode).toBe(403);
        expect(res._getJSONData()).toEqual({ message: "Real-Estate inputs are wrong", err: "Validation Error" });
        expect(realEstateSchema.safeParse).toHaveBeenCalledWith(mockBody);
    });
