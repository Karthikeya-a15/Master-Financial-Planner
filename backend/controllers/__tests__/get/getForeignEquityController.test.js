...
describe("getForeignEquityController", () => {
  it("should return foreign equity data for a valid user", async () => {
    ...
    expect(ForeignEquity.findById).toHaveBeenCalledWith(
      mockUser.netWorth.foreignEquity,
    );
    expect(
      ForeignEquity.findById(mockUser.netWorth.foreignEquity).select,
  ...
  it("should return an error message if foreign equity data is not found", async () => {
    ...
    expect(res._getJSONData()).toEqual({
      message: "Error while Fetching Foreign Equity ",
    });
  });
