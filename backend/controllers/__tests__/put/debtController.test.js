import debtController from "../../put/debtController.js";
...
describe("debtController", () => {
  it("should update debt data for a valid user, valid input, and valid selection", async () => {
    ...
    debtSchema.safeParse.mockImplementation(
      ({
        liquidFund,
        fixedDeposit,
        debtFunds,
        ...
        if (liquidFund && typeof liquidFund === "number")
          return { success: true };
        if (fixedDeposit && typeof fixedDeposit === "number")
          return { success: true };
        if (debtFunds && typeof debtFunds === "number")
          return { success: true };
        if (governmentInvestments && typeof governmentInvestments === "number")
          return { success: true };
    ...
    expect(res._getJSONData()).toEqual({
      message: "Debt updated successfully to Networth",
    });
  ...
  it("should return 403 if input is invalid", async () => {
    ...
    debtSchema.safeParse.mockReturnValue({
      success: false,
