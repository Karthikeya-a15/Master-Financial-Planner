...
import getRealEstateController from "./getRealEstateController.js";
...
describe("getRealEstateController", () => {
    it("should return real estate data for a valid user", async () => {
        ...
        expect(RealEstate.findById(mockUser.netWorth.realEstate).select).toHaveBeenCalledWith('-_id -__v');
    ...
    it("should return an error message if real estate data is not found", async () => {
        ...
        expect(res.statusCode).toBe(200);
        expect(res._getJSONData()).toEqual({ message: "Error while Fetching Real Estate " });
    });
