...
export default function DirectStocks({
    ...
    const newTimeout = setTimeout(async () => {
      ...
      try {
        const response = await axios.get(
          `/api/v1/realtime/autoSuggestionStocks?name=${value}`
        );
  ...
  const totalValue = Object.values(summaryData).reduce(
    (sum, val) => sum + val,
    0
  );
                  ...
                  <ul className="absolute z-10 bg-white border border-gray-300 mt-1 w-full shadow-lg rounded-md">
                    {suggestions.map((suggestion, i) => (
                      <li
                        ...
                        onClick={() => {
                          setNewStock({ ...newStock, stockName: suggestion });
      ...
      {editedData[section].length > 0 ? (
        ...
        >
          ...
          <div className="overflow-x-auto">
              ...
              <tbody className="bg-white divide-y divide-secondary-200">
                {editedData[section].map((stock, index) => (
                  <tr
                    key={index}
                    className={
                      editingStockIndex === index ? "bg-primary-50" : ""
                    }
                  >
                    <td className="px-6 py-4 whitespace-nowrap relative">
                      {editingStockIndex === index ? (
                        ...
                        <div className="text-sm font-medium text-secondary-900">
                          {capitalizeWords(stock.stockName)}
                        </div>
                      )}
                    </td>
            ...
            {!showAddStockForm ? (
              <button
                onClick={() => handleSave("directStocks")}
