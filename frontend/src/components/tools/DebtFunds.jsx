...
function calculateWeightedScores(funds, weightage, expectedInterestRateChange) {
    ...
    funds = funds.map((fund) => {
        fund.expectedReturns = Number(customRound(fund.modifiedDuration * -1 * expectedInterestRateChange + fund.avgYTM - fund.expenseRatio));
        return fund;
    });

    const expectedReturnsRanks = rankByParameter(funds, 'expectedReturns', false);

    const { cagrRanksRatio, volatalityRankRatio, tenureRankRatio, expectedReturns } = weightage;
    ...
    const weightedScores = funds.map(fund => {
        const weightedScore = 
            ...
            expectedReturns * expectedReturnsRanks[fund.name];
...
export default function DebtFunds() {
    ...
    const fetchFunds = async () => {
        try {
            setLoading(true);
            const response = await axios.get('/api/v1/tools/debtfunds');
            setFunds(response.data);
            setLoading(false);
        } catch (error) {
            console.error('Error fetching debt funds:', error);
            setError('Failed to load debt funds data');
            setLoading(false);
            toast.error('Failed to load debt funds data');
        }
    };
    ...
    const handleExpectedInterestRateChange = (e) => {
        const value = parseFloat(e.target.value);
        if (!isNaN(value)) {
            setExpectedInterestRateChange(value);
        }
    };
    ...
    const getSortedFunds = () => {
        ...
        return calculateWeightedScores(funds[selectedCategory], weightage, expectedInterestRateChange);
    };
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="space-y-8">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-2xl font-bold text-secondary-900">Debt Funds Analysis</h1>
                        <p className="mt-1 text-secondary-600">Compare debt funds based on returns and risk metrics</p>
                    </div>
                ...
                >
                    <h2 className="text-xl font-bold text-secondary-900 mb-6">Select Debt Fund Category</h2>
                    <select
                ...
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="form-label">CAGR Rank Weight</label>
                            <div className="flex items-center">
                                <input
                                    ...
                                    max="1"
                                    step="0.1"
                                    value={weightage.cagrRanksRatio}
                                    onChange={(e) => handleWeightageChange('cagrRanksRatio', e.target.value)}
                        ...
                        <div>
                            <label className="form-label">Volatility Weight</label>
                            <div className="flex items-center">
                                <input
                                    type="range"
                                    min="0"
                                    max="1"
                                    step="0.1"
                                    value={weightage.volatalityRankRatio}
                                    onChange={(e) => handleWeightageChange('volatalityRankRatio', e.target.value)}
                                    className="w-full h-2 bg-secondary-200 rounded-lg appearance-none cursor-pointer"
                                />
                                <input
                                    type="number"
                        ...
                        <div>
                            <label className="form-label">Fund Manager Tenure Weight</label>
                            <div className="flex items-center">
                                <input
                                    type="range"
                                    min="0"
                                    max="1"
                                    step="0.1"
                                    value={weightage.tenureRankRatio}
                                    onChange={(e) => handleWeightageChange('tenureRankRatio', e.target.value)}
                                    className="w-full h-2 bg-secondary-200 rounded-lg appearance-none cursor-pointer"
                                />
                                <input
                                    type="number"
                        ...
                        <div>
                            <label className="form-label">Expected Returns Weight</label>
                            <div className="flex items-center">
                                <input
                                    type="range"
                                    min="0"
                                    max="1"
                                    step="0.1"
                                    value={weightage.expectedReturns}
                                    onChange={(e) => handleWeightageChange('expectedReturns', e.target.value)}
                                    className="w-full h-2 bg-secondary-200 rounded-lg appearance-none cursor-pointer"
                                />
                                <input
                                    type="number"
                ...
                {/* Expected Interest Rate Change Input */}
                <motion.div 
                    className="card"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5 }}
                >
                    <h2 className="text-xl font-bold text-secondary-900 mb-6">Expected Interest Rate Change</h2>
                    <div className="flex items-center">
                        <input
                            type="number"
                            value={expectedInterestRateChange}
                            onChange={handleExpectedInterestRateChange}
                            className="w-full p-2 border border-secondary-200 rounded-lg"
                            step="0.1"
                        />
                    </div>
